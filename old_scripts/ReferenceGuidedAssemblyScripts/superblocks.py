...
with open(block_file) as b_f:
    ...
    for row in progressbar.progressbar(read_b_f):
        ...
        all_block_scaf.append(row[0])
        all_block_start.append(int(row[1]))
        all_block_end.append(int(row[2]))
        ...
        if block_scaf[0] == block_scaf[-1]:
            ...
            #    block_end = []
            #    block_scaf = []
                block_start = all_block_start[-1:]
                block_end = all_block_end[-1:]
                block_scaf = all_block_scaf[-1:]
                i=2
                while (block_end[-1] - block_start[0])<300:
                    block_start = all_block_start[-i:]
                    block_end = all_block_end[-i:]
                    block_scaf = all_block_scaf[-i:]
                    i+=1
                    #print("Processing rows! %s" %row)
                pass
        elif block_scaf[0] != block_scaf[-1]: # to stop sp block at the end of chromosome
