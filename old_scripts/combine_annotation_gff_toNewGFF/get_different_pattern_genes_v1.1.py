...
parser.add_argument('-diff_14','--diff_14', type=str, metavar='', required=True, help='cuffdiff1_4')
parser.add_argument('-diff_46','--diff_46', type=str, metavar='', required=True, help='cuffdiff4_6')
parser.add_argument('-diff_69','--diff_69', type=str, metavar='', required=True, help='cuffdiff6_9')
...
diff_46_file = args.diff_46
diff_46_df = pd.read_table(diff_46_file, delimiter="\t", comment="#", names=["test_id","gene_id","gene","locus","sample_1","sample_2","status","value_1","value_2","log2fold_change","test_stat","p_value","q_value","significant"], low_memory=False)
diff_69_file = args.diff_69
diff_69_df = pd.read_table(diff_69_file, delimiter="\t", comment="#", names=["test_id","gene_id","gene","locus","sample_1","sample_2","status","value_1","value_2","log2fold_change","test_stat","p_value","q_value","significant"], low_memory=False)
...
diff_46_df_1 = np.array(diff_46_df)
diff_69_df_1 = np.array(diff_69_df)
...
steep_down_46=np.empty((0, 11))
steep_down_69=np.empty((0, 11))
steep_up=np.empty((0, 11))
steep_up_14=np.empty((0, 11))
steep_up_46=np.empty((0, 11))
steep_up_69=np.empty((0, 11))
rest=np.empty((0, 11))
...
for i in progressbar.progressbar(genes_df_np[:]):
    ...
    #steep down 14 46 69
    if (float(all_stages[0,7])>float(all_stages[1,7]) or (almost_equal(float(all_stages[0,7]),float(all_stages[1,7]),5))):
        if (float(all_stages[1,7])>float(all_stages[2,7]) or almost_equal(float(all_stages[1,7]),float(all_stages[2,7]),5)):
            if (float(all_stages[2,7])>float(all_stages[3,7]) or almost_equal(float(all_stages[2,7]),float(all_stages[3,7]),5)):
                if (float(all_stages[3,7])>float(all_stages[4,7]) or almost_equal(float(all_stages[3,7]),float(all_stages[4,7]),5)):
                    if (float(all_stages[4,7])>float(all_stages[5,7]) or almost_equal(float(all_stages[4,7]),float(all_stages[5,7]),5)):
                        if (float(all_stages[5,7])>float(all_stages[7,7]) or almost_equal(float(all_stages[5,7]),float(all_stages[7,7]),5)):
                            if (float(all_stages[7,7])>float(all_stages[8,8]) or almost_equal(float(all_stages[7,7]),float(all_stages[8,8]),5)):
                                if (diff_69_df_1[diff_69_df_1[:,1]==i,13]=="yes" and diff_14_df_1[diff_14_df_1[:,1]==i,13]=="yes" and diff_46_df_1[diff_46_df_1[:,1]==i,13]=="yes"):
    ...
    #steep down 14
    if (float(all_stages[0,7])>float(all_stages[1,7])):
        if (float(all_stages[1,7])>float(all_stages[2,7])):
            if (float(all_stages[2,7])>float(all_stages[3,7])):
                if (float(all_stages[3,7])>float(all_stages[4,7]) or almost_equal(float(all_stages[3,7]),float(all_stages[4,7]),5)):
                    if (float(all_stages[4,7])>float(all_stages[5,7]) or almost_equal(float(all_stages[4,7]),float(all_stages[5,7]),5)):
                        if (float(all_stages[5,7])>float(all_stages[7,7]) or almost_equal(float(all_stages[5,7]),float(all_stages[7,7]),5)):
                            if (float(all_stages[7,7])>float(all_stages[8,8]) or almost_equal(float(all_stages[7,7]),float(all_stages[8,8]),5)):
                                if (diff_14_df_1[diff_14_df_1[:,1]==i,13]=="yes"):
                                    all_stages_update = np.append(i, all_stages[:,7].T)
                                    all_stages_update = np.append(all_stages_update, all_stages[8,8])
    ...
    if (float(all_stages[0,7])>float(all_stages[1,7]) or (almost_equal(float(all_stages[0,7]),float(all_stages[1,7]),5))):
        if (float(all_stages[1,7])>float(all_stages[2,7]) or almost_equal(float(all_stages[1,7]),float(all_stages[2,7]),5)):
            if (float(all_stages[2,7])>float(all_stages[3,7])):
                if (float(all_stages[3,7])>float(all_stages[4,7])):
                    if (float(all_stages[4,7])>float(all_stages[5,7]) or almost_equal(float(all_stages[4,7]),float(all_stages[5,7]),5)):
                        if (float(all_stages[5,7])>float(all_stages[7,7]) or almost_equal(float(all_stages[5,7]),float(all_stages[7,7]),5)):
                            if (float(all_stages[7,7])>float(all_stages[8,8]) or almost_equal(float(all_stages[7,7]),float(all_stages[8,8]),5)):
                                if (diff_46_df_1[diff_46_df_1[:,1]==i,13]=="yes"):
                                    all_stages_update = np.append(i, all_stages[:,7].T)
                                    all_stages_update = np.append(all_stages_update, all_stages[8,8])
                                    steep_down_46=np.append(steep_down_46,all_stages_update.reshape(1,11),axis=0)
    else:
        all_stages_update = np.append(i, all_stages[:,7].T)
        all_stages_update = np.append(all_stages_update, all_stages[8,8])
        rest=np.append(rest,all_stages_update.reshape(1,11),axis=0)
    ...
    if (float(all_stages[0,7])>float(all_stages[1,7]) or almost_equal(float(all_stages[0,7]),float(all_stages[1,7]),5)):
        if (float(all_stages[1,7])>float(all_stages[2,7]) or almost_equal(float(all_stages[1,7]),float(all_stages[2,7]),5)):
            if (float(all_stages[2,7])>float(all_stages[3,7]) or almost_equal(float(all_stages[2,7]),float(all_stages[3,7]),5)):
                if (float(all_stages[3,7])>float(all_stages[4,7]) or almost_equal(float(all_stages[3,7]),float(all_stages[4,7]),5)):
                    if (float(all_stages[4,7])>float(all_stages[5,7])):
                        if (float(all_stages[5,7])>float(all_stages[7,7])):
                            if (float(all_stages[7,7])>float(all_stages[8,8])):
                                if (diff_69_df_1[diff_69_df_1[:,1]==i,13]=="yes"):
                                    all_stages_update = np.append(i, all_stages[:,7].T)
                                    all_stages_update = np.append(all_stages_update, all_stages[8,8])
                                    steep_down_69=np.append(steep_down_69,all_stages_update.reshape(1,11),axis=0)
    else:
        all_stages_update = np.append(i, all_stages[:,7].T)
        all_stages_update = np.append(all_stages_update, all_stages[8,8])
        rest=np.append(rest,all_stages_update.reshape(1,11),axis=0)
                        ...
                        if (float(all_stages[5,7])<float(all_stages[7,7]) or almost_equal(float(all_stages[5,7]),float(all_stages[7,7]),5)):
                            if (float(all_stages[7,7])<float(all_stages[8,8]) or almost_equal(float(all_stages[7,7]),float(all_stages[8,8]),5)):
                                if (diff_46_df_1[diff_46_df_1[:,1]==i,13]=="yes"):
                                    all_stages_update = np.append(i, all_stages[:,7].T)
                                    all_stages_update = np.append(all_stages_update, all_stages[8,8])
                                    steep_up_46=np.append(steep_up_46,all_stages_update.reshape(1,11),axis=0)
    else:
        all_stages_update = np.append(i, all_stages[:,7].T)
        all_stages_update = np.append(all_stages_update, all_stages[8,8])
        rest=np.append(rest,all_stages_update.reshape(1,11),axis=0)
                        ...
                        if (float(all_stages[5,7])<float(all_stages[7,7])):
                            if (float(all_stages[7,7])<float(all_stages[8,8])):
                                if (diff_69_df_1[diff_69_df_1[:,1]==i,13]=="yes"):
                                    all_stages_update = np.append(i, all_stages[:,7].T)
                                    all_stages_update = np.append(all_stages_update, all_stages[8,8])
                                    steep_up_69=np.append(steep_up_69,all_stages_update.reshape(1,11),axis=0)
...
for i in progressbar.progressbar(int_genes_np[:,1]):
    all_stages = diff_all_df_1[diff_all_df_1[:,1]==i]
    all_stages_update = np.append(i, all_stages[:,7].T)
    all_stages_update = np.append(all_stages_update, all_stages[8,8])
...
steep_up_46_pd = pd.DataFrame(steep_up_46, columns = ['gene_id','s1','s2','s3','s4','s5','s6','s7','s8','s9','s10'])
steep_up_69_pd = pd.DataFrame(steep_up_69, columns = ['gene_id','s1','s2','s3','s4','s5','s6','s7','s8','s9','s10'])
...
steep_down_46_pd = pd.DataFrame(steep_down_46, columns = ['gene_id','s1','s2','s3','s4','s5','s6','s7','s8','s9','s10'])
steep_down_69_pd = pd.DataFrame(steep_down_69, columns = ['gene_id','s1','s2','s3','s4','s5','s6','s7','s8','s9','s10'])
...
with open(str(cwd + '_' + 'steep_up_46.out'), 'w') as out_file:
    steep_up_46_pd.to_csv(out_file, sep="\t", header=True, index=False)

with open(str(cwd + '_' + 'steep_up_69.out'), 'w') as out_file:
    steep_up_69_pd.to_csv(out_file, sep="\t", header=True, index=False)
...
with open(str(cwd + '_' + 'steep_down_46.out'), 'w') as out_file:
    steep_down_46_pd.to_csv(out_file, sep="\t", header=True, index=False)

with open(str(cwd + '_' + 'steep_down_69.out'), 'w') as out_file:
    steep_down_69_pd.to_csv(out_file, sep="\t", header=True, index=False)
...
def plot_graph(in_data,int_gene_data_pd,text):
    #### plot ####
    # style
...
plot_graph(steep_down_pd,int_gene_data_pd,"steep_down_pd")
plot_graph(steep_down_14_pd,int_gene_data_pd,"steep_down_14_pd")
plot_graph(steep_down_46_pd,int_gene_data_pd,"steep_down_46_pd")
plot_graph(steep_down_69_pd,int_gene_data_pd,"steep_down_69_pd")
plot_graph(steep_up_pd,int_gene_data_pd,"steep_up_pd")
plot_graph(steep_up_14_pd,int_gene_data_pd,"steep_up_14_pd")
plot_graph(steep_up_46_pd,int_gene_data_pd,"steep_up_46_pd")
plot_graph(steep_up_69_pd,int_gene_data_pd,"steep_up_69_pd")