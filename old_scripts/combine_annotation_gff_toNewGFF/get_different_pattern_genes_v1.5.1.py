...
#gene_list = args.list
#genes_df = pd.read_csv(gene_list, delimiter="\t", comment="#", names=["gene"], low_memory=False)
...
def almost_equal(a,b,x):
    ...
    if((a<=upper1) and (a>=lower1)) or ((b<=upper2) and (b>=lower2)) or (a==b):
        ...
        return False
total_failed = 0
for i in tqdm(list(diff_all_df.iloc[:,1].unique())):
    #print (i)
    all_stages = diff_all_df_1[diff_all_df_1[:,1]==i]
    all_values = np.append(i, all_stages[:,7].T)
    all_values = np.append(all_values, all_stages[8,8])
    value_count = 0
    ...
    if (diff_14_df_1[diff_14_df_1[:,1]==i,13]=="yes"): # significant between q1 q4
        if float(all_stages[0,7])>=1.0 and float(all_stages[4,7])<1.0 and float(all_stages[5,7])<=1.0 and float(all_stages[7,7])<=1.0 and float(all_stages[8,8])<=1.0:# set lower/upper limits 
            if (float(all_stages[0,7])<float(all_stages[1,7]) and (diff_all_df_1[diff_all_df_1[:,1]==i,13][0]!="yes")) or (float(all_stages[0,7])>=float(all_stages[1,7])):
                if (float(all_stages[1,7])<float(all_stages[2,7]) and (diff_all_df_1[diff_all_df_1[:,1]==i,13][1]!="yes")) or (float(all_stages[1,7])>=float(all_stages[2,7])):
                    if (float(all_stages[2,7])<float(all_stages[3,7]) and (diff_all_df_1[diff_all_df_1[:,1]==i,13][2]!="yes")) or (float(all_stages[2,7])>=float(all_stages[3,7])):
                        if (float(all_stages[3,7])<float(all_stages[4,7]) and (diff_all_df_1[diff_all_df_1[:,1]==i,13][3]!="yes")) or (float(all_stages[3,7])>=float(all_stages[4,7])):
    ...
    if (float(all_stages[8,8])>=5*(float(all_stages[0,7]))) and float(all_stages[3,7])>=1 and float(all_stages[4,7])>=1 and float(all_stages[5,7])>=1 and float(all_stages[7,7])>=1 and float(all_stages[8,8])>=1:
        if (diff_14_df_1[diff_14_df_1[:,1]==i,13]=="yes") and (diff_47_df_1[diff_47_df_1[:,1]==i,13]=="yes"):
            if (float(all_stages[0,7])>float(all_stages[1,7]) and (diff_all_df_1[diff_all_df_1[:,1]==i,13][0]!="yes")) or (float(all_stages[0,7])<=float(all_stages[1,7])):
                if (float(all_stages[1,7])>float(all_stages[2,7]) and (diff_all_df_1[diff_all_df_1[:,1]==i,13][1]!="yes")) or (float(all_stages[1,7])<=float(all_stages[2,7])):
                    if (float(all_stages[2,7])>float(all_stages[3,7]) and (diff_all_df_1[diff_all_df_1[:,1]==i,13][2]!="yes")) or (float(all_stages[2,7])<=float(all_stages[3,7])):
    ...
    #steep up very low to very high but later
        if float(all_stages[0,7])<=1.0 and float(all_stages[1,7])<=1.0 and float(all_stages[2,7])<=1.0 and float(all_stages[3,7])<=1.0 and float(all_stages[4,7])<=1.0 and float(all_stages[5,7])>=1.0:
...
def plot_graph(in_data,text):
    ...
    plt.title(text)
    plt.xlabel("Weeks during vernalization")
