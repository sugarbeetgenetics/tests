#usage
#python3 /media/pflanz/Hs1-2/Scripts/combine_annotation_gff_toNewGFF/get_different_pattern_genes_v1.3.py 
# -diff_all /media/pflanz/Avneesh/Brassicanapus/gatk/second_Attempt/gene_exp_filtered.diff
#  -diff_14 13_gene_exp.diff -diff_47 36_gene_exp.diff -diff_78 68_gene_exp.diff -diff_89 89_gene_exp.diff -diff_79 78_gene_exp.diff 
...
parser.add_argument('-diff_47','--diff_47', type=str, metavar='', required=True, help='cuffdiff4_7')
parser.add_argument('-diff_78','--diff_78', type=str, metavar='', required=True, help='cuffdiff7_8')
parser.add_argument('-diff_89','--diff_89', type=str, metavar='', required=True, help='cuffdiff8_9')
parser.add_argument('-diff_79','--diff_79', type=str, metavar='', required=True, help='cuffdiff7_9')
...
for i in tqdm(genes_df_np[:]):
    ...
    ### up
        ...
        if ((float(all_stages[0,7])>=0 and float(all_stages[0,7])<=1.5) and float(all_stages[8,8])>=10*(float(all_stages[0,7]))):
            if (diff_14_df_1[diff_14_df_1[:,1]==i,13]=="yes") and (diff_47_df_1[diff_47_df_1[:,1]==i,13]=="yes"):
                if (float(all_stages[0,7])>float(all_stages[1,7]) and (diff_all_df_1[diff_all_df_1[:,1]==i,13][0]!="yes")) or (float(all_stages[0,7])<=float(all_stages[1,7])):
                    if (float(all_stages[1,7])>float(all_stages[2,7]) and (diff_all_df_1[diff_all_df_1[:,1]==i,13][1]!="yes")) or (float(all_stages[1,7])<=float(all_stages[2,7])):
                        if (float(all_stages[2,7])>float(all_stages[3,7]) and (diff_all_df_1[diff_all_df_1[:,1]==i,13][2]!="yes")) or (float(all_stages[2,7])<=float(all_stages[3,7])):
        ...
        if ((float(all_stages[0,7])>=0 and float(all_stages[0,7])<=1.5) and (float(all_stages[1,7])>=0 and float(all_stages[1,7])<=1.5) and (float(all_stages[2,7])>=0 and float(all_stages[2,7])<=1.5) and (float(all_stages[3,7])>=0 and float(all_stages[3,7])<=1.5) and (float(all_stages[4,7])>=0 and float(all_stages[4,7])<=1.5) and (float(all_stages[5,7])>=0 and float(all_stages[5,7])<=1.5)):
...
plot_graph(steep_down_14_only_pd,int_gene_data_pd,"steep_down_14_only_pd")
plot_graph(steep_down_14_pd,int_gene_data_pd,"steep_down_14_pd")
plot_graph(steep_down_47_pd,int_gene_data_pd,"steep_down_47_pd")
plot_graph(steep_down_79_pd,int_gene_data_pd,"steep_down_79_pd")
plot_graph(steep_up_pd,int_gene_data_pd,"steep_up_pd")
plot_graph(steep_up_vlTOvh_pd,int_gene_data_pd,"steep_up_vlTOvh_pd")
plot_graph(steep_up_vlTOvh_l_pd,int_gene_data_pd,"steep_up_vlTOvh_l_pd")
plot_graph(steep_up_14_pd,int_gene_data_pd,"steep_up_14_pd")
plot_graph(steep_up_47_pd,int_gene_data_pd,"steep_up_47_pd")
plot_graph(steep_up_79_pd,int_gene_data_pd,"steep_up_79_pd")