...
$usage .= "The gff-file needs to be sorted such that for each gene or mRNA the exons are in increasing order.\n";
$usage .= "options:\n";
...
while (<GFFFILE>) {
	...
	if (($fkey eq "CDS" || $fkey eq "mRNA" || $fkey eq "UTR") && 
		...
		# just add the exon
		push @{$annotation->{$fkey.$genename}}, ($begin, $end);  # TODO: insert at the right place when exons are not sorted in input file
	    } else {
		my @newgbfkey;
