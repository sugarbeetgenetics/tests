######## improvements #########
# 1. can adjust height and width of out image
...
line_items_df = pd.read_csv(str(linesFile), sep="\t", comment="#", low_memory=False, names=["name","length","marker","category"])
line_items_df.marker = line_items_df.marker.astype(str) #make marker column as string
...
for id_name,line,marking in tqdm(combined_data_np):
    ...
    mark_split = nested_change(mark_split_0, float)
